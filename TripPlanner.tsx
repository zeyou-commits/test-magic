...
export function TripPlanner({ selection, onSelect }: { selection: Selection | null; onSelect: (selection: Selection | null) => void }) {
  ...
  const suggestedTripPairs = useMemo(() => {
    ...
    return getSchoolBreaksForZone(zone).flatMap((period) => {
      ...
      const returnCandidates = departures.flatMap((departure) => {
        ...
        if (!from || !to || !isAlgeriaPort(from) || !isFrancePort(to)) return [];
        const date = departure.departure_at.slice(0, 10);
        // Le retour se cherche autour de la fin des vacances, pas sur toute la période.
        // Cela permet notamment de faire remonter les départs depuis TOUS les ports algériens
        // disponibles dans la fenêtre ± flexDays.
        if (date < addDays(period.end, -flexDays) || date > addDays(period.end, flexDays)) return [];
        return [{ departure, route, from, to, period }];
      }).sort((a, b) => {
        const target = toDate(period.end).getTime();
        return Math.abs(toDate(a.departure.departure_at.slice(0, 10)).getTime() - target) -
  ...
  return (
    <div className="border-b border-[#e9dfcf] bg-gradient-to-br from-[#e7f5f4] via-[#fffdf8] to-[#f5ede0]">
      <button
        type="button"
      ...
      {open ? (
        <div className="space-y-4 px-5 pb-4">
          ...
          <section className="space-y-2">
            <div className="grid grid-cols-2 gap-1 rounded-xl border border-[#e0d5c3] bg-[#eee5d7] p-1">
              <button type="button" onClick={() => { setTripMode("roundtrip"); setReturnDate(returnDate || addDays(outboundDate, 7)); }}
                className={`rounded-lg px-3 py-2 text-xs font-semibold ${tripMode === "roundtrip" ? "bg-white text-[#0b7285] shadow-sm" : "text-[#647a80]"}`}>Aller-retour</button>
              <button type="button" onClick={() => { setTripMode("oneway"); setReturnDate(""); }}
                className={`rounded-lg px-3 py-2 text-xs font-semibold ${tripMode === "oneway" ? "bg-white text-[#0b7285] shadow-sm" : "text-[#647a80]"}`}>Aller simple</button>
            </div>
          ...
          <Button
            type="button"
            className="w-full rounded-xl bg-[#e97962] text-white hover:bg-[#d96851]"
            onClick={submit}
          ...
          {schoolInfo ? (
            <div className="rounded-xl border border-[#f2b65e]/30 bg-[#f2b65e]/15 px-3 py-2 text-xs text-[#765821]">
...
function Recommendation({ title, leg, from, to, date, selection, onSelect }: { title: string; leg: LegRecommendation; from: string; to: string; date: string; selection: Selection | null; onSelect: (selection: Selection | null) => void }) {
  return (
    <button type="button" onClick={() => onSelect(selection?.type === "route" && selection.id === leg.route.id ? null : { type: "route", id: leg.route.id })} className="w-full rounded-2xl border border-[#e3d7c4] bg-white/90 p-3 text-left shadow-sm transition hover:border-[#0b7285]/40 hover:bg-[#fffdf8]">
...
function EmptyRecommendation({ text }: { text: string }) {
  return <div className="rounded-xl border border-dashed border-[#d5c8b5] bg-white/60 px-3 py-2 text-xs text-[#647a80]">{text}</div>;
}
