  ...
  return (
    <div className="pb-2">
      <div className="space-y-1">
        <div className={hidePrimarySearchOnMobile ? "hidden md:block" : undefined}>
          <Section title="Rechercher un port">
            <div className="relative">
              <Search className="pointer-events-none absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-[#60767b]" />
              <Input
              ...
              {filters.search ? (
                <button
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full p-1 text-[#647a80] hover:bg-[#f1eadc]"
                >
                  <X className="size-4" />
                </button>
              ) : null}
            ...
            {matchingPorts.length > 0 ? (
              <ul className="mt-2 overflow-hidden rounded-xl border border-[#0b7285]/10 bg-white">
                {matchingPorts.map((port) => (
                  <li key={port.id} className="border-b border-[#e5e9e5] last:border-0">
                    <button
                      type="button"
                      onClick={() => onSelect({ type: "port", id: port.id })}
                      className="flex w-full items-center justify-between gap-3 px-3 py-2.5 text-left hover:bg-[#f5f0e5]"
                    >
                      <span className="min-w-0">
                        <span className="block text-sm font-medium text-[#163a43]">{port.name}</span>
                        <span className="block text-xs text-[#60767b]">
                          {port.city ? `${port.city} · ${port.country_name}` : port.country_name}
                        </span>
                      </span>
                      <span className="shrink-0 text-xs font-semibold text-[#0b7285]">Voir la fiche</span>
                    </button>
                  </li>
                ))}
              </ul>
            ...
            {filters.search && matchingPorts.length === 0 ? (
              <p className="mt-2 text-xs text-[#60767b]">Aucun port trouvé.</p>
            ) : null}
          </Section>
        </div>
          ...
          action={
            hasFilters ? (
              <Button
                ...
                size="sm"
                onClick={reset}
                className="h-8 text-xs text-[#547078] hover:bg-[#f1eadc]"
              >
        ...
        >
          <div className="rounded-xl border border-[#0b7285]/15 bg-white p-2.5 shadow-[0_3px_12px_rgba(18,52,59,0.05)]">
            <div className="mb-1.5 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <MapPinned className="size-3.5 text-[#0b7285]" />
              ...
              {selectedPorts.length ? (
                <span className="rounded-full bg-[#f5f0e5] px-2 py-1 text-[10px] font-semibold text-[#0b7285]">
            ...
            <div>
              <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-[0.1em] text-[#60767b]">
                Pays
              </p>
                  ...
                  return (
                    <button
                      ...
                      className={`inline-flex min-h-8 items-center gap-1.5 rounded-full border px-2.5 text-[11px] font-semibold transition ${
                        selected
                          ? "border-[#0b7285] bg-[#0b7285] text-white shadow-sm"
                          : "border-[#dbe5e2] bg-[#fffdf8] text-[#3f6269] hover:border-[#0b7285]/40 hover:bg-[#eef7f6]"
                      }`}
                    >
            ...
            <div>
              <div className="mb-2 flex items-center justify-between gap-2">
                <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#60767b]">
                  Ports
                </p>
                {activeCountries.length ? (
                  <span className="text-[10px] text-[#60767b]">
                  ...
                  return (
                    <button
                      ...
                      className={`flex min-h-8 items-center justify-between gap-2 rounded-lg border px-2 text-left text-xs transition ${
                        selected
                          ? "border-[#0b7285]/40 bg-[#e7f4f3] text-[#0b6072]"
                          : "border-[#e0e6e3] bg-[#fffdf8] text-[#294b53] hover:border-[#0b7285]/30 hover:bg-[#eef7f6]"
                      }`}
                    >
                      ...
                      {selected ? <Check className="size-4 shrink-0 text-[#0b7285]" /> : null}
                    </button>
                  );
                })}
              </div>
          ...
          <div className="mt-2">
            <Button
              ...
              size="sm"
              className="h-8 rounded-full border-[#0b7285]/15 bg-white px-3 text-[11px] text-[#315860] hover:bg-[#f5f0e5]"
              onClick={() => setFiltersOpen((open) => !open)}
            >
              <SlidersHorizontal className="mr-1.5 size-3.5" />
              Affiner
              {advancedFilterCount > 0 ? (
                <span className="ml-1.5 rounded-full bg-[#e97962] px-1.5 py-0.5 text-[10px] font-bold text-white">
                  {advancedFilterCount}
                </span>
              ) : null}
            </Button>
          ...
          {filtersOpen ? (
            <div className="mt-2 grid gap-2 rounded-xl border border-[#0b7285]/10 bg-[#f5f0e5] p-2.5 sm:grid-cols-2">
              <FilterSelect
                label="Compagnie"
...
{!selection ? (
        <Section
          ...
          action={
            upcoming.length ? (
              <span className="text-[10px] font-medium text-[#60767b]">
        ...
        >
          {departuresLoading ? (
            <div className="flex items-center gap-2 py-2 text-xs text-[#60767b]">
              <CalendarDays className="size-3.5 animate-pulse text-[#0b7285]" />
              ...
              {upcoming.map((departure) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-center gap-2.5 rounded-xl border border-[#e5e9e5] bg-[#fffdf8] px-2.5 py-2 text-left transition hover:border-[#0b7285]/25 hover:bg-[#eef7f6]"
                  >
                    <div className="min-w-[46px] rounded-lg bg-[#e7f4f3] px-1.5 py-1 text-center">
                      <div className="text-[9px] font-semibold uppercase text-[#0b7285]">
                        {date.toLocaleDateString("fr-FR", { weekday: "short" }).replace(".", "")}
                      </div>
                      <div className="text-sm font-bold leading-none text-[#163a43]">
                        {date.toLocaleDateString("fr-FR", { day: "2-digit", month: "2-digit" })}
                      </div>
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-xs font-semibold text-[#163a43]">
                        {departurePort?.name ?? "—"} → {arrivalPort?.name ?? "—"}
                      </div>
                      <div className="truncate text-[10px] text-[#60767b]">
                        {company?.name ?? "Compagnie"}{vessel?.name ? ` · ${vessel.name}` : ""}
                      </div>
                    </div>
                    <div className="shrink-0 text-sm font-bold text-[#0b7285]">
  ...
  return (
    <label className="grid gap-1">
      ...
      <Select value={value ?? ANY} onValueChange={(next) => onChange(next === ANY ? null : next)}>
        <SelectTrigger className="border-[#0b7285]/15 bg-white">
