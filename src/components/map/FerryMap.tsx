...
function routeFeatures(routes: RouteLine[], ports: Map<string, Port>, visible: Set<string>, focus: Set<string>, selection: Selection | null) {
  ...
  return visibleRoutes.map((route) => {
    ...
    return { type: "Feature" as const, properties: { id: route.id, color: routeColor(route.color, from.slug), label: isDisplayRoute && durationMinutes ? formatDuration(durationMinutes) : "", selected: selectedRouteId === route.id, focused: focus.has(route.id) || (isDisplayRoute && focusedPair), dimmed }, geometry: { type: "LineString" as const, coordinates: [[from.longitude, from.latitude], [to.longitude, to.latitude]] } };
...
export default function FerryMap({ ports, routes, visibleRouteIds, focusRouteIds, selection, highlightedPortIds, portMeta, onSelect }: FerryMapProps) {
  ...
  useEffect(() => {
    ...
    const map = new MapLibreMap({ container, style: MAP_STYLE, center: [4.0, 39.0], zoom: isMobile ? 2.9 : 4.8, pitch: isMobile ? 0 : 30, bearing: 0, attributionControl: { compact: true }, fadeDuration: 0 });
    ...
    map.on("load", () => {
      ["label_other", "label_village", "label_town", "label_city", "label_city_capital", "label_country_1", "label_country_2", "label_country_3"].forEach((layerId) => { if (map.getLayer(layerId)) map.setLayoutProperty(layerId, "visibility", "none"); });
      ...
      const pickRoute = (event: MapLayerMouseEvent) => { const id = event.features?.[0]?.properties?.["id"]; if (typeof id === "string") selectRef.current({ type: "route", id }); };
      ...
      map.on("click", (event: MapMouseEvent) => { const hits = map.queryRenderedFeatures(event.point, { layers: ["ferry-routes-line", "ferry-routes-duration"] }); if (hits.length === 0) selectRef.current(null); });
  ...
  useEffect(() => {
    ...
    const repositionVisibleTips = () => { portMarkersRef.current.forEach((marker) => { const element = marker.getElement(); const tip = element.querySelector<HTMLDivElement>(".port-tip"); if (!tip) return; const visible = element.matches(":hover, [data-active=\"true\"]") || element.matches(":focus-within"); if (visible) positionPortTip(map, marker, tip); }); };
    ports.forEach((port) => {
      ...
      if (existing) marker = existing;
        ...
        const dot = document.createElement("button"); dot.type = "button"; dot.className = "port-marker__dot"; dot.setAttribute("aria-label", port.name);
        const label = document.createElement("span"); label.className = "port-marker__label"; label.textContent = port.name;
        const tip = document.createElement("div"); tip.className = "port-tip";
        ...
        dot.addEventListener("click", select); label.addEventListener("click", select);
      ...
      if (label) { Object.assign(label.style, getPortLabelStyle(port)); label.style.fontSize = isSingleLineAlgerianPort ? `${Math.max(6, Math.min(12, 6 + (map.getZoom() - 4.8) * 2))}px` : "12px"; label.style.fontWeight = isSingleLineAlgerianPort ? "500" : "700"; }
      element.dataset["active"] = String(activeIds.has(port.id)); element.dataset["dimmed"] = String(dimming && !activeIds.has(port.id)); element.dataset["closed"] = String(port.status === "inactive");
      marker.setLngLat([port.longitude, port.latitude]);
      const tip = element.querySelector<HTMLDivElement>(".port-tip"); if (!tip) return;
      tip.textContent = "";
      const title = document.createElement("p"); title.className = "port-tip__title"; const country = port.country_name ? `, ${port.country_name}` : ""; const lineLabel = routeCount === 0 ? "aucune ligne visible" : routeCount === 1 ? "1 ligne" : `${routeCount} lignes`; title.textContent = `${port.name}${country}, ${lineLabel}`; tip.append(title);
      if (port.status === "inactive") { const closed = document.createElement("p"); closed.className = "port-tip__closed"; closed.textContent = "Temporairement fermé"; tip.append(closed); }
      if (meta?.companies?.length) { const companies = document.createElement("p"); companies.className = "port-tip__meta"; companies.textContent = `Compagnies : ${meta.companies.slice(0, 3).join(", ")}${meta.companies.length > 3 ? ` +${meta.companies.length - 3}` : ""}`; tip.append(companies); }
      if (meta?.vessels?.length) { const vessels = document.createElement("p"); vessels.className = "port-tip__meta"; vessels.textContent = `Navires : ${meta.vessels.slice(0, 3).join(", ")}${meta.vessels.length > 3 ? ` +${meta.vessels.length - 3}` : ""}`; tip.append(vessels); }
      if (meta?.upcoming?.length) {
        const heading = document.createElement("p"); heading.className = "port-tip__meta port-tip__departures-title"; heading.textContent = "Prochains départs"; tip.append(heading);
        const list = document.createElement("ul"); list.className = "port-tip__departures";
        meta.upcoming.forEach((item) => { const row = document.createElement("li"); row.textContent = `${item.label}${item.to ? ` → ${item.to}` : ""}`; list.append(row); }); tip.append(list);
        if (meta.hasMore) { const more = document.createElement("button"); more.type = "button"; more.className = "port-tip__more"; more.textContent = "Voir plus →"; more.addEventListener("click", (event) => { event.stopPropagation(); selectRef.current({ type: "port", id: port.id }); }); tip.append(more); }
      } else { const next = document.createElement("p"); next.className = "port-tip__next"; next.textContent = "Prochain départ non connu"; tip.append(next); }
      requestAnimationFrame(() => { if (element.matches(":hover, [data-active=\"true\"]") || element.matches(":focus-within")) positionPortTip(map, marker, tip); });
    });
    const handleMapMove = () => repositionVisibleTips();
    map.on("move", handleMapMove); window.addEventListener("resize", repositionVisibleTips);
    portMarkersRef.current.forEach((marker, id) => { if (!ports.some((port) => port.id === id)) { marker.remove(); portMarkersRef.current.delete(id); } });
    return () => { map.off("move", handleMapMove); window.removeEventListener("resize", repositionVisibleTips); };
  }, [ports, highlightedPortIds, portMeta, mapReady]);

  useEffect(() => {
    const map = mapRef.current; if (!map || !mapReady) return;
    const singleLineAlgerianSlugs = new Set(["ghazaouet", "mostaganem", "skikda", "annaba"]);
    const updateAlgerianLabelSizes = () => { const zoom = map.getZoom(); const fontSize = Math.max(6, Math.min(12, 6 + (zoom - 4.8) * 3)); ports.forEach((port) => { if (!singleLineAlgerianSlugs.has((port.slug ?? "").toLowerCase())) return; const marker = portMarkersRef.current.get(port.id); const label = marker?.getElement().querySelector<HTMLSpanElement>(".port-marker__label"); if (label) label.style.fontSize = `${fontSize}px`; }); };
    updateAlgerianLabelSizes(); map.on("zoom", updateAlgerianLabelSizes); return () => { map.off("zoom", updateAlgerianLabelSizes); };
  }, [ports, mapReady]);

  useEffect(() => {
    const map = mapRef.current; if (!map) return;
    const portMap = new Map(ports.map((port) => [port.id, port])); const visible = new Set(visibleRouteIds); const focus = new Set(focusRouteIds);
    const apply = () => { const source = map.getSource("ferry-routes") as GeoJSONSource | undefined; if (!source) return; source.setData({ type: "FeatureCollection", features: routeFeatures(routes, portMap, visible, focus, selection) }); };
